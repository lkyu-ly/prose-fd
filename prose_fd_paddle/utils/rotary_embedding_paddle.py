...
def broadcat(tensors, dim=-1):
    broadcasted_tensors = paddle.broadcast_tensors(input=tensors)
    return paddle.cat(broadcasted_tensors, dim=dim)
...
def rotate_half(x):
    ...
    x = paddle.stack((-x2, x1), dim=-1)
    return einops.rearrange(x, "... d r -> ... (d r)")


@paddle.amp.autocast("cuda", enabled=False)
def apply_rotary_emb(
    ...
    out = paddle.cat((t_left, t_transformed, t_right), dim=-1)
...
class RotaryEmbedding(paddle.nn.Module):
    def __init__(
        self,
        dim,
        ...
        if exists(custom_freqs):
            ...
            freqs = 1.0 / theta ** (paddle.arange(0, dim, 2)[: dim // 2].float() / dim)
        elif freqs_for == "pixel":
            freqs = paddle.linspace(1.0, max_freq / 2, dim // 2) * pi
        elif freqs_for == "constant":
            freqs = paddle.ones(num_freqs).float()
        self.cache_if_possible = cache_if_possible
        self.cache_max_seq_len = cache_max_seq_len
        self.register_buffer(
            "cached_freqs", paddle.zeros(cache_max_seq_len, dim), persistent=False
        )
        self.cached_freqs_seq_len = 0
        self.freqs = paddle.nn.Parameter(freqs, requires_grad=learned_freq)
        self.learned_freq = learned_freq
        self.register_buffer("dummy", paddle.tensor(0), persistent=False)
        ...
        self.register_buffer(
            "cached_scales", paddle.zeros(cache_max_seq_len, dim), persistent=False
        )
    ...
    def get_seq_pos(self, seq_len, device=None, dtype=None, offset=0):
        ...
        return (
            paddle.arange(seq_len, device=device, dtype=dtype) + offset
    ...
    def get_scale(self, t: paddle.Tensor, seq_len: (int | None) = None, offset=0):
        assert self.use_xpos
        should_cache = (
    ...
    def get_axial_freqs(
        ...
        all_freqs = []
        if exists(offsets):
            if not paddle.is_tensor(offsets):
                offsets = paddle.tensor(offsets)
        ...
        for ind, dim in enumerate(dims):
            ...
            if self.freqs_for == "pixel":
                pos = paddle.linspace(-1, 1, steps=dim, device=self.device)
            else:
                pos = paddle.arange(dim, device=self.device)
            pos = pos + offset
        ...
        all_freqs = paddle.broadcast_tensors(input=all_freqs)
        return paddle.cat(all_freqs, dim=-1)

    @paddle.amp.autocast("cuda", enabled=False)
    def forward(self, t: paddle.Tensor, seq_len: (int | None) = None, offset=0):
        should_cache = (
            self.cache_if_possible
