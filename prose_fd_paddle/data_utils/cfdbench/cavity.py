...
class CavityFlowDataset(CfdDataset):
    ...
    data_delta_time = 0.1
    case_params_keys = ["vel_top", "density", "viscosity", "height", "width"]
    ...
    def __getitem__(
        ...
        if self.sample_point_by_point:
            ...
            y = frame_idx // width
            x = frame_idx % width
            case_features = self.features[case_id]
            case_params = self.case_params[case_id]
            query_point = paddle.tensor([t, x, y]).float()
            label = case_features[t, :, y, x]
...
class CavityFlowAutoDataset(CfdAutoDataset):
    ...
    def load_data(self, case_dirs: List[Path], time_step_size: int):
        ...
        self.case_ids = np.array(all_case_ids)

    def __getitem__(
        self, idx: int
    ) -> Tuple[paddle.Tensor, paddle.Tensor, Dict[str, paddle.Tensor]]:
        """
        Return:
            feat: (2, h, w)
            label: (2, h, w)
